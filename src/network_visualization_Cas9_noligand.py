...
structure_id = "4cmp"
filename = "4cmp.pdb" # PDB file containing Cas9 + sgRNA, target DNA crystal structure
...
for i in range(len(protein)):
    try:
        residue = protein[i+4] # Cas9 starts with a Lys3 (Res 1 and 2 undefined, 1-indexed)
        # Gets 3D location of residue based on alpha carbon position
        ca = residue["CA"].get_vector()
        residue_info = [residue.get_resname()+str(i+4), ca[0], ca[1], ca[2]]
        locations.append(residue_info) # Add residue to location array

        # Creates node for residue in 2-D network, sets color
        networkscore_residue = NetworkScores.get(residue.get_resname()+str(i+4))
        residue_name = residue.get_resname()+str(i+4)
...
ax.set_axis_off()
ax.set_title("2D Representation of Cas9 Structural Network (no ligands)")
ax.margins(x=0, y=0)

plt.savefig("4cmp_network_"+structure_id+".pdf", bbox_inches = 'tight',
    pad_inches = 0.5)